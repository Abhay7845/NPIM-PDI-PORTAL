  ...
  const UpdateReportsPdtDetails = async () => {
      ...
      try {
        ...
        if (response.data.code === "1000") {
          ...
          const catPbDataUpper = isCatPB.filter(
            (item_1) =>
              item_1.catPB.toUpperCase() === productsData.catPB.toUpperCase()
          );
          const catPbWiseData = catPbDataUpper.filter(
            (item_2) =>
              item_2.catPB.replace(/\s+/g, "").trim() ==
              productsData.catPB.replace(/\s+/g, "").trim()
          );
    ...
    const TagQunatityUCPData = filteredTags.map((item) => {
      ...
      const unitCost = stdUcpVal[1]
        ? Number(stdUcpVal[1])
        : Number(productsData[costKey]);
      const quantity = Number(item.quantity);
      const set2TagUnitCost =
        item.size === "Set2Tag" && stdUcpVal[1]
          ? Number(stdUcpVal[1])
          : Number(productsData.stdUcpN) + stdUcpVal[1]
          ? Number(stdUcpVal[1])
          : Number(productsData.stdUcpE);
      const set2Tag_HUnitCost =
        item.size === "Set2Tag_H" && stdUcpVal[1]
          ? Number(stdUcpVal[1])
          : Number(productsData.stdUcpH) + stdUcpVal[1]
          ? Number(stdUcpVal[1])
          : Number(productsData.stdUcpE);
      return {
        size: item.size,
    ...
    const TagQunatityStdWtData = filteredTags.map((item) => {
      ...
      const set2TagUnitCost =
        item.size === "Set2Tag" &&
        Number(productsData.stdWtE) + Number(productsData.stdWtN);
      const set2Tag_HUnitCost =
        item.size === "Set2Tag_H" &&
        Number(productsData.stdWtE) + Number(productsData.stdWtH);
      return {
        size: item.size,
        quantity:
          (item.size === "Set2Tag" && set2TagUnitCost * quantity) ||
    ...
    let UmoSizeLimit = 0;
    const bangle11Digit =
      productsData.category === "BANGLE" || productsData.category === "BANGLES"
        ? productsData.itemCode.charAt(10)
        : productsData.childNodeV.charAt(10);
    if (SizableTag.includes("Only_BANGLE")) {
      const singleBanglePrice =
        Number(
          parseFloat(
            stdUcpVal[1] ? stdUcpVal[1] : productsData.stdUcpV
          ).toFixed(1)
      ...
      const singleBanglePrice =
        Number(
          parseFloat(stdUcpVal[1] ? stdUcpVal[1] : productsData.stdUCP).toFixed(
            1
          )
        ) / Number(bangle11Digit) || 1;
      const sizeUomQuantityPrise = allDataFromValidation.sizeUomQuantityRes.map(
    ...
    if (SizableTag.includes("Only_BANGLE")) {
      const singleBanglePrice =
        Number(productsData.stdWtV) / Number(bangle11Digit) || 1;
      const sizeUomQuantityPrise = allDataFromValidation.sizeUomQuantityRes.map(
        (item) => {
          const updatedItem = { size: item.size };
          for (const key in item) {
      ...
      const singleBanglePrice =
        Number(productsData.stdWt) / Number(bangle11Digit) || 1;
      const sizeUomQuantityPrise = allDataFromValidation.sizeUomQuantityRes.map(
        (item) => {
          const updatedItem = { size: item.size };
          for (const key in item) {
    ...
    if (SizableTag.includes("Only_FINGERRING")) {
      sizeUCPLimit =
        SizeQuntyTotal *
        parseFloat(
          Number(stdUcpVal[1] ? stdUcpVal[1] : productsData.stdUcpF).toFixed(2)
        );
    } else if (SizableTag.includes("Only_MANGALSUTRA")) {
      sizeUCPLimit =
        SizeQuntyTotal *
        parseFloat(
          Number(stdUcpVal[1] ? stdUcpVal[1] : productsData.stdUcpN).toFixed(2)
        );
    } else {
      sizeUCPLimit =
        SizeQuntyTotal *
        parseFloat(
          Number(stdUcpVal[1] ? stdUcpVal[1] : productsData.stdUCP).toFixed(2)
        );
    }
    ...
    if (SizableTag.includes("Only_FINGERRING")) {
      TotalsizeStdWt =
        SizeQuntyTotal * parseFloat(Number(productsData.stdWtF).toFixed(2));
    } else if (SizableTag.includes("Only_MANGALSUTRA")) {
      TotalsizeStdWt =
        SizeQuntyTotal * parseFloat(Number(productsData.stdWtN).toFixed(2));
    } else {
      TotalsizeStdWt =
        SizeQuntyTotal * parseFloat(Number(productsData.stdWt).toFixed(2));
    }
    ...
    const indQuntyLimit =
      Number(allDataFromValidation.quantityRes) *
      Number(stdUcpVal[1] ? stdUcpVal[1] : productsData.stdUCP);

    // <------------- INPUT CALCULATION FOR STD ------------------------------->
    const InputStdWt =
      Number(allDataFromValidation.quantityRes) * Number(productsData.stdWt);
