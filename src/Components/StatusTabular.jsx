...
const StatusTabular = ({ statusData }) => {
  ...
  const columns =
    storeValue !== "L3" && statusData.col.length > 0
      ? [...statusData.col, "REMAINING SKU COUNT"]
      : [...statusData.col];
  const TableData = statusData.row.map((eachItem) => {
    return {
      id: eachItem.id,
      consumerBase: eachItem.needState
        ? eachItem.needState
        : eachItem.consumerBase,
      totalSKU: eachItem.totalSku !== "N/A" ? eachItem.totalSku : "",
      feedbackGiven:
        eachItem.totalSku !== "N/A" || eachItem.feedBackGiven !== "N/A"
          ? parseInt(
              eachItem.feedBackGiven
                ? eachItem.feedBackGiven
                : eachItem.saleable
            )
          : "",
      remainingSKUcount:
        eachItem.totalSku !== "N/A" || eachItem.feedBackGiven !== "N/A"
          ? (
              parseInt(
                eachItem.totalSku ? eachItem.totalSku : eachItem.totalSKU
              ) - parseInt(eachItem.feedBackGiven ? eachItem.feedBackGiven : 0)
            ).toString()
          : "",
    };
  });
          ...
          <TableBody>
            {statusData.row &&
              TableDetails.map((statusColoum, index) => (
                <TableRow key={index}>{rowcreater(statusColoum)}</TableRow>
              ))}
          </TableBody>
        </Table>
      ) : (
