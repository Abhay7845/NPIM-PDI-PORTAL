...
import {
  APIGetDropdownCollectionList,
  APIGetDropdownConsumerBaseList,
  APIGetDropdownITGroupList,
  APIGetDropdownList,
...
const LowerHeaderDigital = (props) => {
  ...
  const onchangeHandler = (event) => {
    ...
    if (name === "collection") {
      setLoading(true);
      APIGetDropdownCollectionList(
        `/NPIM/base/npim/dropdown/${value}/ALL/ALL/ALL`
      )
        .then((res) => res)
        .then((response) => {
      ...
      setLoading(true);
      APIGetDropdownConsumerBaseList(
        `/NPIM/base/npim/dropdown/${dropState.collection}/${value}/ALL/ALL`
      )
        .then((response) => {
      ...
      setDropValueForCategoryState([]);
      APIGetDropdownITGroupList(
        `/NPIM/base/npim/dropdown/${dropState.collection}/${dropState.consumerBase}/${value}/ALL`
      )
        .then((response) => {
          if (response.data.code === "1000") {
                  ...
                  <div className="d-flex">
                    ...
                    {loginData.role === "L3" && (
                      <div
                        ...
                        onClick={() =>
                          navigate(
                            `/NpimPortal/wishlist/${storeCode}/${rsoName}`
                          )
                        }
                      >
              ...
              {loginData.role === "L3" && (
                <div
                  ...
                  onClick={() => {
                    if (CartItemCode) {
                      navigate(
                        `/NpimPortal/cart/product/L3/${storeCode}/${rsoName}`
                      );
