...
const AddedCartTable = () => {
  ...
  useEffect(() => {
    ...
    if (editProductsData.itemCode) {
      GetProductDetailsBySearchPnpim(editProductsData.itemCode);
    }
  ...
  const MoveToWishlist = (event) => {
    setLoading(true);
    APIMoveToWishList(
      `/NPIML3/npim/move/item/wishlist/to/indent/${event.itemCode}/${storeCode}/Wishlist`
    )
      .then((res) => res)
      .then((response) => {
        if (response.data.Code === "1000") {
  ...
  const DataRows = rows.filter(
    (item) =>
      item.itemCode.includes(inputItemCode.toUpperCase()) ||
  ...
  const HandelClickYes = () => {
    setLoading(true);
    APIYesItemWiseRtp(
      `/NPIML3/npim/item/wise/rpt/edr/L3/${storeCode}`,
      DataRows
    )
      .then((res) => res)
      .then((response) => {
    ...
    <React.Fragment>
      ...
      {rows.length > 0 && cols.length > 0 ? (
        <div className="mx-2 my-2">
          ...
          <div className="row d-flex justify-content-between mx-0 my-2">
            <div className="col-md-4">
              <input
                ...
                className="mb-1"
                onChange={(e) => setInputItemCode(e.target.value)}
              />
            </div>
            <div className="d-flex col-md-4">
              <h6 className="mx-2">
                Total: <b>{DataRows.length}</b>
              </h6>
              <h6 className="mx-2">
                Successful Indent Count: <b>{successCount.length}</b>
              </h6>
            </div>
          ...
          {editProductsData.id && (
            <EditItemWiseProducts
              itemWiseData={itemWiseData}
              rows={DataRows}
              productsData={editProductsData}
              AlertPopupStatus={setAlertPopupStatus}
              ItemWiseReport={GetItemWiseReports}
              editProductsData={setEditProductsData}
            />
          )}
          <DataGrid
            rows={DataRows}
        ...
        <h5 className="text-center text-danger my-2">DATA NOT AVAILABLE</h5>
      )}
    </React.Fragment>
  );
};
