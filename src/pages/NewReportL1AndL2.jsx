...
const NewReportL1AndL2 = () => {
  ...
  const collType = pndReports.map((item) =>
    item.collection.split(/\s+/).join("")
  );
  ...
  useEffect(() => {
    const CollFilter = pndReports.filter(
      (item) =>
        item.collection &&
        item.collection.split(/\s+/).join("").trim() === collectionOpt
    );
  ...
  const getProductData = (data) => {
    ...
    setLoading(true);
    APIPNPIMProductData(
      `/NPIM/base/npim/get/product/details`,
      getItemByCardDataNpim
    )
      .then((res) => res)
      .then((response) => {
        if (response.data.code === "1000") {
      ...
      <Grid item xs={12}>
        {productInfo.id && (
          <NewEditProductL1L2
            ...
            GetL1l2PendinRtp={GetL1l2PendinRtp}
